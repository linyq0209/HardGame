...
public class Pooler 
{
    ...
    public static GameObject GetPoolObj(string name)
    {
        ...
        GameObject go = pools[name];
        // .transform.childCount <= 0 ? MonoBehaviour.Instantiate(prefabMap[name]) : pools[name].transform.GetChild(0).gameObject;
        // go.SetActive(true);
        return go;
    }
    /// <summary>
